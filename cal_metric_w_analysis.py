...
def cal_em_accuracy(dataset, jsonl_path):
    ...
    search_time = []
    s_t_correct_dict = {}       
    for i in range(20): # initialize s_t_correct_dict up to 20
        s_t_correct_dict[i] = (0, 0)
    
    test_ids = []
    with open(f'./dataset/{dataset}/{dataset}_test_ids.txt', "r") as f:
        # read the first line only
        first_line = f.readline().strip().split(",")
        test_ids.extend(first_line)
    ...
    with open(jsonl_path, "r") as f:
        for line in f:
            if line.strip():  # skip empty lines
                ...
                s_t = result["search_times"]
                search_time.append(s_t)
                s_t_correct_dict[s_t] = (s_t_correct_dict[s_t][0] + compute_score, s_t_correct_dict[s_t][1] + 1)
    assert len(search_time) == n_examples
    if len(search_time) == 0:
        print(f"[Warning] No examples found in {jsonl_path}!")
        return
    print("max search time:", max(search_time))
    accuracy = (total_score / n_examples) if n_examples > 0 else 0
    avg_search_time = (sum(search_time) / n_examples) if n_examples > 0 else 0
    print(f"[{dataset}] EM accuracy: {total_score}/{n_examples} = {accuracy:.3f}", f"\t Average search times: {avg_search_time:.3f}")
    print("-"*40)
    for s_t in range(max(search_time)+1):
        correct, total = s_t_correct_dict[s_t]
        acc = (correct / total) if total > 0 else 0
        print(f"Search times = {s_t}: {correct}/{total} = {acc:.3f}")
    print("\n")
...
if __name__ == "__main__":
    ...
    datasets = ["cora", "arxiv", "products", "sports", "computers", "pubmed"]
    retriever_types = ["raw", "hop", "hop-global"]
    do_print = 0
    ...
    if datasets is None or len(datasets) == 0:
        ...
        for dataset in datasets:
            ...
            for r_type in retriever_types:
                output_result_path = f"results/{dataset}_32b_{r_type}-similarity_updatedprompt.json" # _enhancedsearchprompt
                print(f"Calculating EM accuracy for {dataset}: {output_result_path} ...")
                cal_em_accuracy(dataset, output_result_path)
                # if r_type == "raw":
                    
                # output_result_path = f"results/{dataset}_32b_{r_type}-similarity_enhancedsearchprompt.json"
                # print(f"Calculating EM accuracy for {dataset}: {output_result_path} ...")
                # cal_em_accuracy(dataset, output_result_path)

                # output_result_path = f"results/{dataset}_32b_{r_type}-similarity_o1.json"
                # print(f"Calculating EM accuracy for {dataset}: {output_result_path} ...")
                # cal_em_accuracy(dataset, output_result_path)
            
                output_result_path = f"results/{dataset}_32b_{r_type}-similarity_titlequery.json"
                print(f"Calculating EM accuracy for {dataset}: {output_result_path} ...")
                cal_em_accuracy(dataset, output_result_path)
