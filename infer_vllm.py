...
def search(query: str, node_id: str, search_cnt: int, curr_nb_ids: List[str]):
    payload = {
            "queries": [query],
    ...
    print("[POST] node_id:", node_id, "curr_nb_ids:", curr_nb_ids, "query:", query)
...
if __name__ == "__main__":
    ...
    for node_id in tqdm(ids_list[args.start_idx:], total=len(ids_list), initial=args.start_idx, desc="Processing"):
        ...
        while True:
            ...
            #     # VLLM
            #     out = outputs[0].outputs[0]    # top hypothesis
            ...
            # if "<answer>" in output_text:
            #     output_text += "</answer>"
            #     print(f"[Enhanced] output_text:", output_text)
            ...
            if tmp_query:
                ...
                if len(retrived_nb_ids) == 0: # no neighbor
                    ...
                    search_results, retrived_nb_ids = search(tmp_query, node_id, search_cnt, retrived_nb_ids)
                # print(f"[Finish] search_results:", search_results, "retrived_nb_ids:", retrived_nb_ids)
            else:
                search_results = ''
            ...
            if search_cnt == 1:
                retrived_nb_ids = [node_id]
            if len(retrived_nb_ids) == 0:
                search_results = 'No relevant information found.\n'
            else:
                earch_results, retrived_nb_ids = search(tmp_query, node_id, search_cnt, retrived_nb_ids)
